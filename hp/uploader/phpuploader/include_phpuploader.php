...
{ 
    ...
    { 
        ...
        {
            $ar[$k] = iconv("UCS-2",$PhpUploader_Encoding,pack("H4",substr($v,-4)));
        }
    } 
    return join("",$ar); 
} 
...
class PhpUploader
{
	...
	function _HandleAddonVerify()
	{
		...
		$newname=PhpUploader_GetQSD("_VFN");
		if($newname&&$newname!=$mvcfile->FileName)
		{
		    $mvcfile->FileName=PhpUploader_GetBaseName($newname);
	...
	function _MoveHttpFile($file,$guid)
	{
		...
		//throw(new Exception("Get file ".$file["name"]." - $filepath"));
